...
#     -   1 for production amplifier, or
#     -   2 for both.
#     (We don't need to unlock 3 of them, because Somersloop analysis is not necessary.)
#     APAs use 10 Somersloop per building, and
#     we also need to take into account the unclocking in MAM, which also costs Somersloops.
# -   First todos.
#     -   Write tests, and compare them to Satisfactory Tools.
#         (Minimizing for relative resource usage, and setting values, should get exactly the same results, even powers.)
#     -   Refactor this resource extractor usages.
#         -   The solve() function should go through each extraction recipe in one loop,
#             not two loops at the beginning.
#         -   Maybe we could change the UI to be more consistent, but we'll see.
#     -   For all resources, differentiate between purity to get exact numbers for consumption too later.
#     -   Add power generators.
#     -   Add all sinks.
# -   Domain related changes.
#     -   Use resource nodes and ingredients, and multiply them in extractors, so we also turn on
#         maximalization for which Mk to use.
#     -   Take into account geothermal.
#         NOTE! This cannot be overclocked.
#     -   Check whether using gross power as maximizing is valid always (especially when using Somersloops),
#         and how can we deal with this in a more elegant way.
#     -   Do a "follow-up" discretization step for when using Somersloops in buildings.
#         In practice, this would be the following: do the linera solution,
#         then fix the amplified recipes to exact amounts to similar values which are the results of the previous solution,
#         and rerun the maximalization problem.
#     -   Remove the allow_production_amplifier, and instead just use the item_min_limits as an indicator.
#     -   Check the following: if for any item (non-power related), if we'd like to set it to a value,
#         does this algorithm also try to minize the power in a weird way? So if it minimizes the power,
#         does this add other buildings there just to consume more power?
#         (It would be because of that epsilon for the c in the solver.)
#     calculate them, and summerize the results.
#     -   Sinking allowed or not.
#     -   Using Alien Power Augmenter or not.
#         -   Fueling them or not.
#     -   Using Somersloops in buildings or not.
#     -   Whether buildings are underclocked to the minimum or default.
#     -   For wells power usage, we can't unfortunately divide the number of wells, because if only one well is used, then it's exact.
#         But it's fine, only a small amount power is used there.
#     -   Analysis of uderclocking (approximately how much we gain/lose).
#     -   For APA, we can maximize for the amplified power with c, because that's linear in gross and net power.
#         -   We can even do this analysis depending on how much fueled and unfueled we use.
#         -   `(power_gross + 500 * APA) * (1 + 0.1 * UAPA + 0.3 * FAPA) - (power_gross - power_net)` max is the same as
#             `power_gross * (1 + 0.1 * UAPA + 0.3 * FAPA) - (power_gross - power_net)` max is the same as
#             `power_gross * (0.1 * UAPA + 0.3 * FAPA) + power_net` max.
#     -   For using somersloops in buildings, we can create two nice graphs with x axis is the number of somersloops used, with
#         -   y axis is different kinds of optimal power used (coal, fuel, etc.), and
#         -   y axis is which recipes are amplified by how much.
#     -   We could also maximize for gross power just out of curiosity.

# Note.
# This way, the only things not taken into account are the
# -   Biomass Burner,
# -   Power Storage.
